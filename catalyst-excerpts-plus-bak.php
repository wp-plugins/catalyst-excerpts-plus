...
= 1.2.5 =
* Added option to display posts from custom post types.
* Added options to use featured image as the background image for the excerpt content.
...
function cep_get_custom_types()
{
	...
	//$dropdown = '<select id="custom-post-type" name="custom-post-type" style="margin-left:15px;">';
	$dropdown = '';
...
class catalyst_excerpt_plus_widget extends WP_Widget {
	...
	function widget( $args, $options )
	{
		...
		if( $options['content-display'] != "tweets" ) {
			...
			//Puts thumbnail outside content
			...
			if( !empty( $options['display-title'] ) )
			{
				if( isset( $options['allow-title-html'] ) ) {
					...
					if( $options['remove-title-link'] == 1){
						printf( '<h2 class="entry-title">%s</h2>', the_title('','',0) );
					}else{
	...
	function catalyst_echo_excerpt_widget_content( $options )
	{
		...
		//Puts thumbnail inside and above content
		...
		//Puts thumbnail inside and below content
		if( function_exists( 'has_post_thumbnail' ) && empty( $options['thumb-background'] ) )
		{
...
{
	$cache_file = plugins_url('data/twitter.txt', __FILE__);
	...
	//if ($cache_file){
	//	echo "<strong>FILE</strong><br /><br /><br /><br /><br />";
		$tweet_found = true;
		// Display tweets from the cache.
		@readfile($cache_file); 

	} else {
	//	echo "<strong>FETCH</strong><br />".$cache_file."<br /><br /><br /><br />";
