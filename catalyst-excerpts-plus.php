		...
		<div style="width:510px; float:left;">
			...
			<div style="background:#F1F1F1 url( <?php echo $image_url ?> ) repeat-x; border:1px solid #E3E3E3; margin-bottom:10px; padding:10px 10px 0;position:relative;">
				...
				<p style="text-align:center;">
					<strong>Catalyst Excerpts Plus 1.3</strong>
				</p>
				<p style="text-align:center;">
					Created by 
				...
				if (count($xml->channel->item)) {
					...
					foreach($xml->channel->item as $tweet) {
						...
						if ($tweet_first_char!='@' || $ignore_replies==false){
							...
							$tweet_desc = strtr($tweet_desc,'<a href="$1">$1</a>','/(https?:\/\/[^\s"<>]+)/'); 
							$tweet_desc = strtr($tweet_desc,'$1<a href="http://twitter.com/$2">@$2</a>','/(^|[\n\s])@([^\s"\t\n\r<:]*)/is');
							$tweet_desc = strtr($tweet_desc,'$1<a href="http://twitter.com/search?q=%23$2">#$2</a>','/(^|[\n\s])#([^\s"\t\n\r<:]*)/is');
