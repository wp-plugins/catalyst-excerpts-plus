...
{
	$cep_widget_name = ''; //Put the new name between the apostrophies ('').
...
class catalyst_excerpt_plus_widget extends WP_Widget {
	...
	function widget( $args, $options )
	{
		...
		elseif( $options['content-display'] == "tweets") 
		{
			...
			$twitwrapopen = '';
			$twitwrapclose = '<div style="clear:both;"></div>';
			$tweetwrapopen = '<div class="catalyst-excerpt-widget' . $options['class'] . '"><div class="catalyst-excerpt-widget-inner"><div class="entry-content"><p><span class="status">';
			$metawrapopen = '</span>' . $tweetmetaplace . '<span class="meta">';
			$metawrapclose = '</span></p>';
			$tweetwrapclose = '</div></div></div><div style="clear:both;"></div>';
